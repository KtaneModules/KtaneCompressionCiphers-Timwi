...
public class HuffmanCipherModule : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        var wordList = new Data().allWords.Skip(1).SelectMany(x => x).ToList();
        _answer = wordList[Rnd.Range(0, wordList.Count)];

        _pages = new string[2][];
        _pages[0] = new string[3];
        _pages[1] = new string[3];
        ...
        treeLogMessages.Add(string.Format("[Huffman Cipher #{0}] Letters on the module: {1}", _moduleId, fakeAlphabetScores));
        ...
        while (nodes.Count > 1)
        {
            ...
            nodes.Add(newNode);
            treeLogMessages.Add(string.Format("[Huffman Cipher #{0}] Combining {1} ({2}) and {3} ({4}), tree is now: {5}",
                _moduleId, lowestScoreNode, lowestScoreNode.Score, secondLowestScoreNode, secondLowestScoreNode.Score, nodes.Join(", ")));
        }
        var root = nodes[0];
        treeLogMessages.Add(string.Format("[Huffman Cipher #{0}] Finished tree: {1}", _moduleId, root));
        ...
        foreach (var letter in _answer)
        {
            ...
            encWordLogMessages.Add(string.Format("[Huffman Cipher #{0}] {1} decodes to {2}", _moduleId, encodedBits.Skip(prevEncodedIx).Join(""), letter));
        ...
        while (encodedBits.Count > 0)
        {
            ...
            if (encodedBits[0] != 0 && (encodedBits[1] != 0 || encodedBits[2] != 0))
            {
                ...
                encBinaryLogMessages.Add(string.Format("[Huffman Cipher #{0}] {1} becomes {2}", _moduleId, encodedWord.Last(), encodedBits.Take(4).Join("")));
            ...
            else
            {
                ...
                encBinaryLogMessages.Add(string.Format("[Huffman Cipher #{0}] {1} becomes {2}", _moduleId, encodedWord.Last(), encodedBits.Take(5).Join("")));
        ...
        foreach (var msg in treeLogMessages)
            Debug.Log(msg);
        Debug.LogFormat("[Huffman Cipher #{0}] Encoded word: {1}", _moduleId, encodedWord);
        foreach (var msg in encBinaryLogMessages)
            Debug.Log(msg);
        Debug.LogFormat("[Huffman Cipher #{0}] String of binary: {1}", _moduleId, encodedBitsStr);
        foreach (var msg in encWordLogMessages)
            Debug.Log(msg);
        Debug.LogFormat("[Huffman Cipher #{0}] Solution word: {1}", _moduleId, _answer);
